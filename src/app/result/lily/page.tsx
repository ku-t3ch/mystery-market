...
export default function LilyPage() {
  ...
  const [message, setMessage] = useState(
    `ถึงเธอที่รักเธอช่างเหมือนดั่งลิลลี่ในยามบ่าย
    คุณมักช่างสงสัยใคร่รู้เและเปิดกว้างกับ
  ...
  return (
    ...
    >
      ...
      <div className="flex h-screen w-screen  backdrop-blur-sm flex-col ">
        ...
        <center>
          <div className="flex flex-col w-[308px] top-[517px]  text-zinc-800 text-base font-normal font-['IBM Plex Sans Thai'] leading-[30px] m-3 text-left">
            {message.split("\n").map((line, index) => (
              <p key={index}>{line}</p>
            ))}
          </div>
